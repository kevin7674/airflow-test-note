...
def test_flow_1():
    ...
    with open('/usr/local/airflow/test_flow_1.txt', 'a') as f:
        f.write('%s\n' % current_time)
...
def test_flow_2():
    current_time = str(datetime.today())
    with open('/usr/local/airflow/test_flow_2.txt', 'a') as f:
        f.write('%s\n' % current_time)

# 定義要執行的Python函數3
def test_flow_3():
    current_time = str(datetime.today())
    with open('/usr/local/airflow/test_flow_3.txt', 'a') as f:
        f.write('%s\n' % current_time)
...
# 定義要執行的task 2
t2 = PythonOperator(
    task_id='test_flow_2',  # task_id
    python_callable=test_flow_2,  # 指定要執行的函数
    dag=dag,  # 指定歸屬的dag
)

# 定義要執行的的task 3
t3 = PythonOperator(
    task_id='test_flow_3',  # task_id
    python_callable=test_flow_3,  # 指定要執行的函数
    dag=dag,  # 指定歸屬的dag
)
...
# 同時等價於 dag.set_dependency('test_flow_1', 'test_flow_2')
